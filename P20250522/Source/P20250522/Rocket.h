...
public:	
	...
	UFUNCTION(BlueprintImplementableEvent) //BluePrint에서 Implemented 될 수 있음, Cpp에서 구현안됨
	void CallCPPToExecuteBP(int Damage);

	UFUNCTION(BlueprintNativeEvent)
	void CallCPPToDefaultExecuteBP(int Damage);
	void CallCPPToDefaultExecuteBP_Implementation(int Damage);  //Cpp에서만 Implemented 됨, Blueprint 에선 보여지지 않음
};
